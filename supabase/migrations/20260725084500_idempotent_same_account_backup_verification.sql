-- Re-importing a valid backup into its unchanged source account is a safe
-- verification operation, not an error. Existing live data is preserved and
-- the client receives the same duplicate-safe result used for repeated imports.
-- Full same-account recovery still requires clearing live finance data first.
  ...
  if v_target_user_id = v_source_owner_id then
    ...
    if v_has_live_finance_data then
      return jsonb_build_object(
        ...
        'added', '{}'::jsonb,
        'skipped', coalesce(
          p_backup#>'{manifest,recordCounts}',
          '{}'::jsonb
        ),
        'restored', '{}'::jsonb,
        'sealed', true
