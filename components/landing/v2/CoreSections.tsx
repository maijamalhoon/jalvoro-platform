      ...
      <div className={`${container} grid lg:grid-cols-3`}>
        {items.map(([Icon, title, copy], index) => {
          ...
          return (
            <article
              key={title}
              className={`flex items-center gap-4 border-b border-[#12211b]/[0.07] py-5 lg:min-h-32 lg:border-b-0 lg:px-10 ${
                index < 2 ? "lg:border-r" : ""
              } ${index === 0 ? "lg:pl-0" : ""}`}
            >
              <RailIcon className="size-6 shrink-0 text-emerald-700" />
      ...
      <div className="mt-12 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {workspaces.map((workspace, index) => {
          ...
          return (
            <article
              key={workspace.name}
              className="group relative flex min-h-[420px] flex-col overflow-hidden rounded-3xl border border-[#12211b]/[0.07] bg-white/90 p-7 transition duration-300 hover:-translate-y-1 hover:border-[#12211b]/15 hover:shadow-[0_22px_48px_rgba(28,55,43,.09)]"
            >
              <span className="absolute -right-20 -top-20 size-52 rounded-full bg-emerald-100/75 transition duration-500 group-hover:scale-105" />
              <span className="absolute right-6 top-6 z-10 text-[11px] font-bold text-[#12211b]/25">
                0{index + 1}
              </span>
              <span className="relative z-10 grid size-12 place-items-center rounded-[14px] bg-emerald-50 text-emerald-700">
                <Icon className="size-[22px]" />
              </span>
              <p className="mt-12 text-xs font-semibold text-slate-500">
                {workspace.label}
              </p>
...
export function WorkflowSection() {
  ...
  return (
    <section
      className={`border-y border-[#12211b]/[0.07] bg-white ${sectionSpace}`}
    >
      <div className={`${container} grid items-center gap-14 xl:grid-cols-[minmax(0,.82fr)_minmax(500px,1.18fr)] xl:gap-[clamp(4rem,7vw,7rem)]`}>
          ...
          <div className="mt-8 grid gap-5">
            {points.map(([Icon, title, copy]) => {
              ...
              return (
                ...
                >
                  <PointIcon className="row-span-2 size-[42px] rounded-xl bg-emerald-50 p-3 text-emerald-700" />
                  <b className="text-sm">{title}</b>
                  <small className="mt-0.5 text-xs leading-5 text-slate-500">
                    {copy}
                  </small>
                </span>
              );
        ...
        <div className="relative overflow-hidden rounded-[30px] border border-[#12211b]/[0.07] bg-[radial-gradient(circle_at_50%_42%,rgba(15,159,110,.09),transparent_34%),#f8fbf9] p-[clamp(1.5rem,4vw,3.4rem)]">
          <div className="grid items-center gap-4 md:grid-cols-[1fr_auto_1fr]">
            <span className="flex min-h-[72px] items-center justify-center gap-2.5 rounded-2xl border border-[#12211b]/[0.07] bg-white p-4 text-center text-xs font-bold shadow-sm">
          ...
          <div className="my-5 grid gap-2.5 md:grid-cols-3">
            {outcomes.map(([Icon, label]) => {
              ...
              return (
                <span
                  key={label}
                  className="flex min-h-20 items-center justify-center gap-2 rounded-2xl border border-[#12211b]/[0.07] bg-white p-3 text-center text-xs font-bold"
                >
                  <FlowIcon className="size-5 text-emerald-700" />
                  {label}
                </span>
              );
            })}
          ...
          <div className="mx-auto flex max-w-xl items-center gap-3 rounded-2xl bg-[#12211b] p-4 text-white shadow-[0_18px_40px_rgba(18,33,27,.18)]">
    ...
    <section id="how-it-works" className={`${container} ${sectionSpace}`}>
      ...
      <div className="mt-14 grid gap-3 md:grid-cols-2 xl:grid-cols-4 xl:gap-0 xl:border-y xl:border-[#12211b]/10">
        {steps.map(([number, title, copy], index) => (
          <article
            key={number}
            className={`min-h-[240px] rounded-2xl border border-[#12211b]/10 p-7 xl:min-h-[280px] xl:rounded-none xl:border-y-0 xl:border-l-0 xl:p-8 ${
          ...
          >
            ...
            <h3 className="mt-12 text-lg font-bold tracking-tight xl:mt-16">
              {title}
            </h3>
...
export function CoverageSection() {
  return (
    <section className={`bg-[#12211b] text-white ${sectionSpace}`}>
        ...
        <div className="mt-12 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          {capabilityGroups.map(([Icon, title, items]) => {
            ...
            return (
              <article
                key={title}
                className="min-h-[320px] rounded-3xl border border-white/10 bg-white/[0.045] p-7"
              >
                ...
                <ul className="mt-5 grid list-none gap-3 text-sm text-slate-300">
                  {items.map((item) => (
                    <li
                      key={item}
