      ...
      <section className="relative w-full max-w-md rounded-[1.8rem] border border-white/10 bg-[#0d1a2d]/95 p-6 shadow-2xl backdrop-blur-xl sm:p-8">
        ...
        <div className="mt-8">
          ...
          <p className="mt-3 text-sm leading-6 text-slate-300">
            Use the email and password registered for your JALVORO Command Center account.
          </p>
        </div>
        ...
        {signedInEmail ? (
          ...
          <form className="mt-7 grid gap-5" onSubmit={signIn}>
            ...
            >
              {busy ? "Authenticating…" : "Open Command Center"}
            </button>
          </form>
        )}

        {signedInEmail && error ? (
          <div role="alert" className="mt-4 rounded-xl border border-red-400/20 bg-red-400/10 px-4 py-3 text-sm leading-6 text-red-100">
            {error}
          </div>
        ) : null}
