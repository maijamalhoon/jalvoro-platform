  ...
  const priorities: Priority[] = [
    ...
    },
    {
      ...
      value: billing.providerConnected
        ? billing.webhooks.pending + billing.webhooks.failed24h
        : "Dormant",
      tone:
        billing.providerConnected && billing.webhooks.failed24h > 0
          ? "critical"
          : billing.providerConnected && billing.webhooks.pending > 0
            ? "attention"
            : "healthy",
    ...
    <section className="cc-exec" aria-labelledby="cc-exec-title">
      <header className="cc-exec-head">
        <div>
          ...
          <p>
            One action-first view of identity, organizations, security,
            reliability, finance operations, governance and release control.
          </p>
        </div>
        <div className="cc-exec-readiness" data-state={release.overall}>
          <span>Platform readiness</span>
          <strong>{release.score}%</strong>
          <small>{release.overall}</small>
        </div>
      </header>
      ...
      <div className="cc-exec-metrics">
        <Metric
          ...
          detail={`${snapshot.users.signedIn24h} signed in during 24h`}
          href="/admin?view=users"
          tone="info"
        />
        <Metric
          label="Active organizations"
          value={organizations.totals.active}
          detail={`${organizations.totals.memberships} total memberships`}
          href="/admin?view=organizations"
        />
        <Metric
          label="Active operators"
          value={activeOperators}
          detail={`${access.counts.pendingInvitations} pending invitations`}
          href="/admin?view=security"
          tone="positive"
        />
        <Metric
          label="Open incidents"
          value={incidents.counts.open + incidents.counts.acknowledged + incidents.counts.investigating + incidents.counts.monitoring}
          detail={`${incidents.counts.criticalOpen} critical`}
          href="/admin?view=reliability"
        ...
        <Metric
          ...
          detail={`${snapshot.billing.pastDueUsers} past due`}
          href="/admin?view=finance"
          tone={snapshot.billing.pastDueUsers > 0 ? "warning" : "positive"}
        />
        <Metric
          label="Active product users"
          value={snapshot.telemetry.activeUsers24h}
          detail={`${snapshot.telemetry.events24h} events in 24h`}
      ...
      <div className="cc-exec-columns">
        <section className="cc-exec-priority" aria-labelledby="cc-priority-title">
          <div className="cc-exec-section-head">
            <div>
              <p className="cc-workspace-kicker">Priority queue</p>
              <h2 id="cc-priority-title">What needs attention now</h2>
            </div>
          ...
          <div className="cc-exec-priority-list">
            {priorities.map((priority) => (
              <Link
                href={priority.href}
                key={priority.label}
                className="cc-exec-priority-row"
        ...
        <section className="cc-exec-coverage" aria-labelledby="cc-coverage-title">
          <div className="cc-exec-section-head">
            <div>
              <p className="cc-workspace-kicker">Operational coverage</p>
              <h2 id="cc-coverage-title">Real data sources</h2>
            </div>
          </div>
          <div className="cc-exec-coverage-list">
            <div data-state="live">
              <span>Identity and authentication</span>
              <strong>{users.counts.totalUsers} accounts</strong>
            </div>
            <div data-state={snapshot.telemetry.telemetrySubjectsStored > 0 ? "live" : "waiting"}>
              <span>Device, location and product telemetry</span>
              <strong>
                {snapshot.privacy.telemetrySubjectsStored > 0
                  ? `${snapshot.privacy.telemetrySubjectsStored} observed users`
                  : "No observations yet"}
              </strong>
            </div>
            <div data-state="live">
              <span>Organization control plane</span>
              <strong>{organizations.totals.total} organizations</strong>
            </div>
            <div data-state={billing.providerConnected ? "live" : "waiting"}>
              <span>Payment provider pipeline</span>
              <strong>{billing.providerConnected ? "Connected" : "Not connected"}</strong>
            </div>
            <div data-state="live">
              <span>Privacy and compliance</span>
              <strong>{snapshot.privacy.openRequests} open requests</strong>
            </div>
            <div data-state={release.runtime.vercel ? "live" : "waiting"}>
              <span>Deployment runtime evidence</span>
              <strong>{release.runtime.vercel ? release.runtime.environment : "Unavailable"}</strong>
            </div>
          </div>
        </section>
      </div>
      ...
      <nav className="cc-exec-actions" aria-label="Primary operating workspaces">
        {[
          ...
          ["Operate reliability", "Incidents, failures and performance", "/admin?view=reliability"],
          ["Run finance ops", "Plans, subscriptions and billing pipeline", "/admin?view=finance"],
          ...
          ["View global topology", "Products, regions, devices and browsers", "/admin?view=operations"],
